...
| M3 `header-footer-1` / `columns-1` / `float-1` targets | Yes, ADR-0031 on non-current contract 1.3 | Yes: all three private vertical slices | No; public IDs rejected until MI3-12 | No, MI3-12 gate |
...
未採択のmath/vector/book publicationは各decision-gate ADRがAcceptedになるまでcontract-definedへ昇格しない。ADR-0031で採択済みのadvanced paginationはMI3-09〜MI3-11でheader/footer、columns、floatをprivate implementationへ進めたが、MI3-12まではpublic/release-supportedへ昇格しない。公開済み`paragraph-1`、`basic-document-1`、`table-1`、`footnote-1`を拡張して代用しない。
