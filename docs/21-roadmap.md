...
| M4 contract 1.4 / `production-book-1` assembled target | Yes through ADR-0037 for base/media, native math/safe-vector, producer-composed math vector, metadata/language/outline, tagged PDF/PDF/UA-1 validation, and distinct `jpeg-baseline`/`sfnt-cff1` resource components | Private implementation is complete through MI4-12 and MI4-V18; MI4-V19 external combined evidence remains | No; current public surface remains 1.3 | No; MI4-V19 then MI4-13 gate |
| remaining M4 evidence/publication and M5 gates | JPEG/CFF contract and private runtimes are complete; M5 decisions remain assigned later | MI4-V19 and MI4-13 remain | No | No |
