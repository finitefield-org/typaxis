...
| M4 contract 1.4 / `production-book-1` assembled target | Yes through ADR-0037 for base/media, native math/safe-vector, producer-composed math vector, metadata/language/outline, tagged PDF/PDF/UA-1 validation, and distinct `jpeg-baseline`/`sfnt-cff1` resource components | Private implementation and MI4-V19 external/readiness evidence are complete | No; current public surface remains 1.3 | No; MI4-13 atomic publication remains |
| remaining M4 publication and M5 gates | JPEG/CFF/vector contract, private runtimes, and feature-local evidence are complete; M5 decisions remain assigned later | MI4-13 remains | No | No |
