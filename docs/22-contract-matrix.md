...
| contract 1.4 / `production-book-1` target | Yes through ADR-0035: base/media, math/safe-vector, metadata/language/outline, and tagged PDF/PDF/UA-1 validation | Private MI4-02 semantic-container/base-media, MI4-04 SafeVector, MI4-05 math, and MI4-07 metadata/language/outline through PDF validation; tagged implementation remains MI4-09 | No; current aliases remain 1.3 | No, MI4-13 gate |
...
| wire ID | current `typaxis_core::CONTRACT` is 1.3; typed public DocumentPackage input IDs are 1.0/1.1/1.2/1.3; ADR-0032 reserves non-current 1.4, ADR-0033 fixes its math/safe-vector extension, ADR-0034 fixes metadata/language/outline, and ADR-0035 adds no new Wire member while fixing the PDF projection | current roots use 1.3; complete 1.0, 1.1, and 1.2 registries are frozen separately; `schemas/1.3/` is current and `schemas/1.4/` is an independent private staging registry | contract-version, ADR-0027 through ADR-0035 | independent frozen 1.0/1.1/1.2 and current/versioned 1.3 registries now; private 1.4 slice validation may grow, while MI4-13 alone switches aliases |
...
| machine package ingestion | stable-byte admission, strict decoder, sealed source validation, and session-bound receipts are implemented; `WireDocumentPackage` remains untrusted; target 1.4 adds semantic container, required declared media, explicit math nodes/source/alternative, safe-vector media, document metadata/language, and explicit outline; ADR-0035 consumes those facts without adding Wire | public 1.0/1.1/1.2/1.3 DocumentPackage input and current 1.3 output; target 1.4 remains public `P1103` | ADR-0027, ADR-0028, ADR-0031 through ADR-0035, docs/02,19,25,26, contracts/phase-ownership | current independent Schema/semantic validation, typed round trip, and public E2E; MI4-02/04/05/07 implement private slices, MI4-09 implements tagging, and MI4-13 is the atomic gate |
| machine PDF capability | exact seven-profile descriptors with matching preflight receipts; ADR-0032/0033/0034/0035 reserve no public descriptor yet | current 1.3 capability Schema and canonical seven-profile fixture; target 1.4 reserves `production-book-1` with a required PDF/UA-1 component | contracts/machine-pdf-capabilities, ADR-0027 through ADR-0035, docs/26 | current bidirectional descriptor/combined-fixture closure and `m3-all.json`; future M4 combined fixture/default/old-profile freeze at MI4-13 |
...
| tagged PDF/accessibility target | source-bound exhaustive roles/generated wrappers in a layout-contract-owned registry and logical order; selected fragment/paint to structure-or-artifact receipt; dense page-local MCIDs; StructTreeRoot/RoleMap/MCR/OBJR/ParentTree/IDTree closure; exact alternatives, language, headers, notes, links, outline `/SE`, and `book-xmp/2`; PDF/UA-1 validation with no warning fallback | Contract-defined by ADR-0035 only; MI4-09 implements the private structure/Display/PDF/manifest/in-tree-validator slice and no current/frozen contract or old profile gains tags | ADR-0035, contracts/contract-version, contracts/machine-pdf-capabilities, contracts/phase-ownership, docs/25 | writer-independent observation plus exact veraPDF 1.30.2 `ua1` and complete Matterhorn 1.1 ledger on one PDF hash; machine-only success is not a full conformance/accessibility claim and MI4-13 owns publication |
...
evidence. MI4-02 through MI4-12 may implement only private contract-1.4
staging, and MI4-13 alone may move the “after” column into public code.
...
human semantic checks or a broader legal/accessibility claim; MI4-13 remains
the only publication/evidence aggregation gate.
