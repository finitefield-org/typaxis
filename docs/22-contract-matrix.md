...
| contract 1.4 / `production-book-1` target | Yes, ADR-0032 base plus ADR-0033 math/safe-vector decision | Private MI4-02 semantic-container/base-media slice only; later M4 slices pending | No; current aliases remain 1.3 | No, MI4-13 gate |
...
| declared-media target | target domain separates provenance-bound `LegacyUnspecified` from typed `Declared`; machine profile owns allowed set; resource admission alone issues and exact-matches attestation; source exporter consumes that same receipt | private 1.4 requires image `png` and fonts `sfnt-truetype-glyf`, `ttc-truetype-glyf`; ADR-0033 additionally fixes `svg-safe-1`, while the M4 manifest keeps declaration and attestation distinct | ADR-0032, ADR-0033, contracts/contract-version, contracts/phase-ownership, docs/25 | MI4-02 base round-trip/mismatch and source-export tests; MI4-04 implements the adopted safe-vector value, MI4-10 owns only JPEG/OTF-CFF additions, and MI4-13 freezes old bytes plus M4 branches |
| math/safe-vector target | exact `typaxis-math` version `1` source/span plus producer speech, opaque MathReceiptKey, admitted MATH font, atomic inline or dedicated display MathFlowId, selected vector paint/ActualText/Formula closure; stable-byte-only bounded Safe-SVG decoder and canonical IR/usage/Form chain | private 1.4 target adds `inline_math`, `display_math`, required source/alternative members, and declared/attested `svg-safe-1`; no old contract/profile gains these shapes | ADR-0033, contracts/machine-pdf-capabilities, contracts/phase-ownership, docs/25 | MI4-04 safe-vector admission/paint and MI4-05 math/source/alternative closure; exact/max+1, forbidden feature, no-external-reference/network, tamper, PDF extraction, public-isolation, and MI4-13 combined gates |
