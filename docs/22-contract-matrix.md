...
| wire ID | current `typaxis_core::CONTRACT` is 1.3; typed public DocumentPackage input IDs are 1.0/1.1/1.2/1.3; ADR-0032 reserves non-current 1.4, ADR-0033 fixes native math/safe-vector, ADR-0034 fixes metadata/language/outline, ADR-0035 fixes the PDF projection, ADR-0036 reserves `jpeg-baseline`/`sfnt-cff1`, and ADR-0037 fixes four producer-vector kinds plus `svg-safe-2` | current roots use 1.3; complete 1.0, 1.1, and 1.2 registries are frozen separately; `schemas/1.3/` is current and `schemas/1.4/` is the independent private staging registry containing the assigned ADR-0032〜0037 product shapes through MI4-12 and MI4-V18 | contract-version, ADR-0027 through ADR-0037 | frozen/current registries now; MI4-V19 closes feature readiness, and MI4-13 alone switches aliases |
...
| machine package ingestion | stable-byte admission, strict decoder, sealed source validation, and session-bound receipts are implemented; `WireDocumentPackage` remains untrusted; target 1.4 adds semantic container, required declared media including `jpeg-baseline`/`sfnt-cff1`/`svg-safe-2`, native and producer-vector math/source/alternative, metadata/language, and outline; ADR-0035/0037 version the PDF/structure projection | public 1.0/1.1/1.2/1.3 DocumentPackage input and current 1.3 output; target 1.4 remains public `P1103`, while its assigned ADR-0032〜0037 Schema/Rust slices are implemented only in private staging | ADR-0027, ADR-0028, ADR-0031 through ADR-0037, docs/02,19,25,26, contracts/phase-ownership | current independent Schema/semantic validation, typed round trip, and public E2E; MI4-V19 precedes the MI4-13 atomic gate |
...
| declared-media target | target domain separates provenance-bound `LegacyUnspecified` from typed `Declared`; machine profile owns allowed set; resource admission alone issues and exact-matches attestation; source exporter consumes that same receipt | private 1.4 requires images `png`, `svg-safe-1`, `svg-safe-2`, or `jpeg-baseline` according to the owning profile and fonts `sfnt-truetype-glyf`, `ttc-truetype-glyf`, or `sfnt-cff1`; manifest declaration and attestation remain distinct | ADR-0032, ADR-0033, ADR-0036, ADR-0037, contracts/contract-version, contracts/phase-ownership, docs/25 | base/SafeVector/JPEG/CFF closure and private `svg-safe-2` wire/admission are implemented; MI4-V19 closes evidence, and MI4-13 freezes old bytes plus complete M4 branches |
...
| producer-composed vector target | explicit `inline_vector`, `math_vector`, `vector_figure`, `math_vector_block`; fixed-point advance/ascent/descent/origin/baseline/viewport and spacing; opaque TeX/Alt/ActualText; Safe-SVG 2 currentColor/alpha; atomic line/block/number layout; verified-content-key Form/ExtGState dedupe; navigation/tagged-PDF `/2` closure | MI4-V01/V02 fix the corpus/decision and MI4-V03〜V19 implement the private product path plus external/readiness evidence | ADR-0037, docs/27, contracts/machine-pdf-capabilities, contracts/phase-ownership | MI4-V19 closes combined negative/tamper/determinism, pinned renderer/extractor/validator, Matterhorn 1.02, and production resource-set `/2`; MI4-13 alone publishes the complete descriptor |
| JPEG/CFF resource target | distinct immutable PNG, SafeVector, baseline-JPEG, TrueType-glyf, and standalone-CFF1 component IDs compose one ordered production resource set; `jpeg-baseline` fixes 8-bit JFIF Gray/YCbCr marker/decode/metadata-strip/DCTDecode policy; `sfnt-cff1` fixes standalone name-keyed CFF1 table/license/glyph/hint-stripped CID-subset/FontFile3 policy; typed IDs bind every receipt | MI4-11 and MI4-12 implement the separate private Schema/Rust admission, transform/subset, PDF, manifest, exact-limit, renderer/extractor, dependency-audit, and old-profile gates; current/frozen/public artifacts remain unchanged | ADR-0036, contracts/contract-version, contracts/machine-pdf-capabilities, contracts/phase-ownership, docs/25 | MI4-13 owns combined advertisement/evidence after MI4-V19 |
...
implemented these paths separately in private staging; public/frozen surfaces
remain isolated until MI4-13.
