...
| producer-composed vector target | explicit `inline_vector`, `math_vector`, `vector_figure`, `math_vector_block`; fixed-point advance/ascent/descent/origin/baseline/viewport and spacing; opaque TeX/Alt/ActualText; Safe-SVG 2 currentColor/alpha; atomic line/block/number layout; verified-content-key Form/ExtGState dedupe; navigation/tagged-PDF `/2` closure | MI4-V01/V02 fix the corpus/decision and MI4-V03〜V18 implement the Wire/Schema/domain/parser/layout/PDF/manifest/capability product path in private staging | ADR-0037, docs/27, contracts/machine-pdf-capabilities, contracts/phase-ownership | MI4-V19 requires combined negative/tamper/determinism plus external renderer/extractor/validator evidence and depends on completed MI4-11/12; MI4-13 alone publishes resource-set `/2` and the complete descriptor |
