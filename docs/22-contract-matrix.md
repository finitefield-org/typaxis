...
| contract 1.4 / `production-book-1` target | Yes through ADR-0035: base/media, math/safe-vector, metadata/language/outline, and tagged PDF/PDF/UA-1 validation | Private MI4-02 semantic-container/base-media, MI4-04 SafeVector, MI4-05 math, MI4-07 metadata/language/outline, and MI4-09 tagged-PDF slices | No; current aliases remain 1.3 | No, MI4-13 gate |
...
| effective config/build manifest | `EffectiveConfig` / `BuildInputProfile` / `PackageInputRecord` / publication contexts | current 1.3 config/build Schema with conditional `advanced_pagination`; private target 1.4 adds production media/math, MI4-07 metadata/language/navigation facts, and MI4-09 tagged observation/evidence facts | docs/16,19,25, ADR-0031 through ADR-0035 | current raw 1.0/1.1/1.2/1.3 normalization and terminal publication; MI4-13 must preserve old raw-contract/profile 1.3 bytes while atomically adding 1.4 version dispatch and forbidding M4 fields in old-profile branches |
