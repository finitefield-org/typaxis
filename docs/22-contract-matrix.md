...
| contract 1.4 / `production-book-1` target | Yes, ADR-0032 base plus ADR-0033 math/safe-vector decision | Private MI4-02 semantic-container/base-media and MI4-04 SafeVector admission/PDF slices; remaining M4 slices pending | No; current aliases remain 1.3 | No, MI4-13 gate |
...
| wire ID | current `typaxis_core::CONTRACT` is 1.3; typed public DocumentPackage input IDs are 1.0/1.1/1.2/1.3; ADR-0032 reserves non-current 1.4 and ADR-0033 fixes its math/safe-vector extension | current roots use 1.3; complete 1.0, 1.1, and 1.2 registries are frozen separately; `schemas/1.3/` is current and `schemas/1.4/` is an independent private staging registry | contract-version, ADR-0027 through ADR-0033 | independent frozen 1.0/1.1/1.2 and current/versioned 1.3 registries now; private 1.4 slice validation may grow, while MI4-13 alone switches aliases |
...
| machine package ingestion | stable-byte admission, strict decoder, sealed source validation, and session-bound receipts are implemented; `WireDocumentPackage` remains untrusted; target 1.4 adds semantic container, required declared media, explicit math nodes/source/alternative, and safe-vector media | public 1.0/1.1/1.2/1.3 DocumentPackage input and current 1.3 output; target 1.4 remains public `P1103` | ADR-0027, ADR-0028, ADR-0031, ADR-0032, ADR-0033, docs/02,19,25,26, contracts/phase-ownership | current independent Schema/semantic validation, typed round trip, and public E2E; MI4-02 implements the private base slice, MI4-04 its SafeVector branch, and MI4-13 is the atomic gate |
| machine PDF capability | exact seven-profile descriptors with matching preflight receipts; ADR-0032/0033 reserve no public descriptor yet | current 1.3 capability Schema and canonical seven-profile fixture; target 1.4 reserves `production-book-1` | contracts/machine-pdf-capabilities, ADR-0027 through ADR-0033, docs/26 | current bidirectional descriptor/combined-fixture closure and `m3-all.json`; future M4 combined fixture/default/old-profile freeze at MI4-13 |
...
| semantic-container target | target owner graph fixes closed `result`, `proof`, `exercise`, NodeId/SourceSpan/child ownership, `semantic_container` style scope, one canonical container FlowId, strict page-fragment progress, and one outline/tag structure boundary | private 1.4 block requires `kind`, `node_id`, `span`, `classes`, `semantic_kind`, and nonempty `blocks`; current/frozen Schema aliases remain unchanged | ADR-0032, contracts/machine-pdf-capabilities, contracts/phase-ownership, docs/25 | MI4-02 private result/proof/exercise, nested/split/empty/unknown/wrong-owner/round-trip/tamper fixtures; MI4-13 public combined closure |
...
| math/safe-vector target | exact `typaxis-math` version `1` source/span plus producer speech, opaque MathReceiptKey, admitted MATH font, atomic inline or dedicated display MathFlowId, selected vector paint/ActualText/Formula closure; stable-byte-only bounded Safe-SVG decoder and canonical IR/usage/Form chain | private 1.4 now implements declared/attested `svg-safe-1` and its SafeVector manifest; `inline_math`, `display_math`, and required source/alternative members remain pending; no old contract/profile gains these shapes | ADR-0033, contracts/machine-pdf-capabilities, contracts/phase-ownership, docs/25 | MI4-04 SafeVector admission/paint exact/max+1, forbidden-feature, no-external-reference/network, tamper, PDF, and public-isolation gates are implemented; MI4-05 owns math/source/alternative closure and MI4-13 the combined publication gates |
