...
| contract 1.4 / `production-book-1` target | Yes, ADR-0032 base, ADR-0033 math/safe-vector, and ADR-0034 metadata/language/outline decisions | Private MI4-02 semantic-container/base-media, MI4-04 SafeVector admission/PDF, and MI4-05 math source/vector/alternative slices; ADR-0034 implementation and remaining M4 slices pending | No; current aliases remain 1.3 | No, MI4-13 gate |
...
| wire ID | current `typaxis_core::CONTRACT` is 1.3; typed public DocumentPackage input IDs are 1.0/1.1/1.2/1.3; ADR-0032 reserves non-current 1.4, ADR-0033 fixes its math/safe-vector extension, and ADR-0034 fixes metadata/language/outline | current roots use 1.3; complete 1.0, 1.1, and 1.2 registries are frozen separately; `schemas/1.3/` is current and `schemas/1.4/` is an independent private staging registry | contract-version, ADR-0027 through ADR-0034 | independent frozen 1.0/1.1/1.2 and current/versioned 1.3 registries now; private 1.4 slice validation may grow, while MI4-13 alone switches aliases |
...
| machine package ingestion | stable-byte admission, strict decoder, sealed source validation, and session-bound receipts are implemented; `WireDocumentPackage` remains untrusted; target 1.4 adds semantic container, required declared media, explicit math nodes/source/alternative, safe-vector media, document metadata/language, and explicit outline | public 1.0/1.1/1.2/1.3 DocumentPackage input and current 1.3 output; target 1.4 remains public `P1103` | ADR-0027, ADR-0028, ADR-0031 through ADR-0034, docs/02,19,25,26, contracts/phase-ownership | current independent Schema/semantic validation, typed round trip, and public E2E; MI4-02/04/05 implement private slices, MI4-07 owns metadata/navigation, and MI4-13 is the atomic gate |
| machine PDF capability | exact seven-profile descriptors with matching preflight receipts; ADR-0032/0033/0034 reserve no public descriptor yet | current 1.3 capability Schema and canonical seven-profile fixture; target 1.4 reserves `production-book-1` | contracts/machine-pdf-capabilities, ADR-0027 through ADR-0034, docs/26 | current bidirectional descriptor/combined-fixture closure and `m3-all.json`; future M4 combined fixture/default/old-profile freeze at MI4-13 |
...
| declared-media target | target domain separates provenance-bound `LegacyUnspecified` from typed `Declared`; machine profile owns allowed set; resource admission alone issues and exact-matches attestation; source exporter consumes that same receipt | private 1.4 requires image `png|svg-safe-1` and fonts `sfnt-truetype-glyf`, `ttc-truetype-glyf`; the M4 manifest keeps declaration and attestation distinct | ADR-0032, ADR-0033, contracts/contract-version, contracts/phase-ownership, docs/25 | MI4-02 base round-trip/mismatch and source-export tests; MI4-04 implements SafeVector stable-byte/IR/manifest closure, MI4-10 owns only JPEG/OTF-CFF additions, and MI4-13 freezes old bytes plus M4 branches |
| math/safe-vector target | exact `typaxis-math` version `1` source/span plus producer speech, opaque MathReceiptKey, admitted MATH font, atomic inline or dedicated display MathFlowId, selected vector paint/ActualText/Formula closure; stable-byte-only bounded Safe-SVG decoder and canonical IR/usage/Form chain | private 1.4 implements declared/attested `svg-safe-1` and its SafeVector manifest plus closed `inline_math` / `display_math` source, speech, MATH-font, vector-paint, selected-placement, PDF `/ActualText`, and manifest facts; no old contract/profile gains these shapes | ADR-0033, contracts/machine-pdf-capabilities, contracts/phase-ownership, docs/25 | MI4-04 SafeVector and MI4-05 math admission/paint, exact/max+1, tamper, PDF, extraction, and public-isolation gates are implemented; MI4-08 fixes Formula structure policy, MI4-09 implements it, and MI4-13 owns combined publication gates |
| metadata/language/outline target | explicit all-null-or-producer metadata, canonical UTC-second dates, registry-independent BCP 47 parse/casing/inheritance over logical owners, explicit source-preorder outline hierarchy, exact package AnchorId and selected named-destination binding, fixed Info/XMP/catalog/outline bytes and observations | ADR-0034 adds required target metadata/document-language/outline and nullable container-anchor shapes; private Schema/Rust/PDF/validator implementation remains MI4-07 and no current/frozen contract or old profile gains the fields | ADR-0034, contracts/contract-version, contracts/machine-pdf-capabilities, contracts/phase-ownership, docs/25 | decision gate fixes exact/max+1 reuse of existing limits, precise JSON Pointer diagnostics, clock/host/path prohibition, receipt tamper closure, and MI4-13 publication isolation; implementation evidence follows in MI4-07 |
...
outline binding. MI4-02 through MI4-12 may implement only private
contract-1.4 staging, and MI4-13 alone may move the “after” column into public
code.
...
inference is forbidden. MI4-07 owns the private Schema/Rust/PDF/validator
implementation without changing current aliases or public descriptors.
