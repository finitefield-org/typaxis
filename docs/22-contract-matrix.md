...
| wire ID | current `typaxis_core::CONTRACT` is 1.3; typed public DocumentPackage input IDs are 1.0/1.1/1.2/1.3 | current roots use 1.3; complete 1.0, 1.1, and 1.2 registries are frozen separately; `schemas/1.3/` is the complete current registry | contract-version, ADR-0027, ADR-0028, ADR-0029, ADR-0030, ADR-0031 | independent frozen 1.0/1.1/1.2 and current/versioned 1.3 registries with no cross-registration |
...
| machine package ingestion | stable-byte admission, strict decoder, sealed source validation, and session-bound receipts are implemented; `WireDocumentPackage` remains untrusted | public 1.0/1.1/1.2/1.3 DocumentPackage input and current 1.3 output, including required trim/region/column/placement members | ADR-0027, ADR-0028, ADR-0031, docs/02,19,25,26, contracts/phase-ownership | independent Schema/semantic validation, typed round trip, and public E2E |
| machine PDF capability | exact seven-profile descriptors with matching preflight receipts; the three ADR-0031 receipts remain session/limits-bound | current 1.3 capability Schema and canonical seven-profile fixture with conditional advanced descriptor members | contracts/machine-pdf-capabilities, ADR-0027, ADR-0028, ADR-0029, ADR-0030, ADR-0031, docs/26 | bidirectional descriptor/combined-fixture closure, old-profile freeze, and `m3-all.json` release gate |
