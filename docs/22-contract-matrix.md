...
| contract 1.4 / `production-book-1` target | Yes through ADR-0036: base/media, math/safe-vector, metadata/language/outline, tagged PDF/PDF/UA-1 validation, and separate baseline-JPEG/CFF1 resource components | Private MI4-02 semantic-container/base-media, MI4-04 SafeVector, MI4-05 math, MI4-07 metadata/language/outline, and MI4-09 tagged-PDF slices; JPEG/CFF implementation remains MI4-11/12 | No; current aliases remain 1.3 | No, MI4-13 gate |
...
| wire ID | current `typaxis_core::CONTRACT` is 1.3; typed public DocumentPackage input IDs are 1.0/1.1/1.2/1.3; ADR-0032 reserves non-current 1.4, ADR-0033 fixes its math/safe-vector extension, ADR-0034 fixes metadata/language/outline, ADR-0035 adds no new Wire member while fixing the PDF projection, and ADR-0036 reserves exact `jpeg-baseline`/`sfnt-cff1` enum additions for private implementation | current roots use 1.3; complete 1.0, 1.1, and 1.2 registries are frozen separately; `schemas/1.3/` is current and `schemas/1.4/` is an independent private staging registry that does not yet contain the ADR-0036 values | contract-version, ADR-0027 through ADR-0036 | independent frozen 1.0/1.1/1.2 and current/versioned 1.3 registries now; private 1.4 slice validation may grow in MI4-11/12, while MI4-13 alone switches aliases |
...
| machine package ingestion | stable-byte admission, strict decoder, sealed source validation, and session-bound receipts are implemented; `WireDocumentPackage` remains untrusted; target 1.4 adds semantic container, required declared media including adopted future `jpeg-baseline`/`sfnt-cff1`, explicit math nodes/source/alternative, safe-vector media, document metadata/language, and explicit outline; ADR-0035 consumes those facts without adding Wire | public 1.0/1.1/1.2/1.3 DocumentPackage input and current 1.3 output; target 1.4 remains public `P1103`, and ADR-0036 enum values are not yet in private Schema/Rust | ADR-0027, ADR-0028, ADR-0031 through ADR-0036, docs/02,19,25,26, contracts/phase-ownership | current independent Schema/semantic validation, typed round trip, and public E2E; MI4-02/04/05/07/09 implement existing private slices, MI4-11/12 add resource slices, and MI4-13 is the atomic gate |
| machine PDF capability | exact seven-profile descriptors with matching preflight receipts; ADR-0032 through ADR-0036 reserve no public descriptor yet | current 1.3 capability Schema and canonical seven-profile fixture; target 1.4 reserves `production-book-1`, required PDF/UA-1, and exact `typaxis.production-book-resource-set/1` component/media arrays | contracts/machine-pdf-capabilities, ADR-0027 through ADR-0036, docs/26 | current bidirectional descriptor/combined-fixture closure and `m3-all.json`; future M4 combined resource/semantic fixture, default, and old-profile freeze at MI4-13 |
...
| declared-media target | target domain separates provenance-bound `LegacyUnspecified` from typed `Declared`; machine profile owns allowed set; resource admission alone issues and exact-matches attestation; source exporter consumes that same receipt | private 1.4 currently requires image `png` or `svg-safe-1` and fonts `sfnt-truetype-glyf`, `ttc-truetype-glyf`; ADR-0036 contract-defines future exact additions `jpeg-baseline` and `sfnt-cff1`; the M4 manifest keeps declaration and attestation distinct | ADR-0032, ADR-0033, ADR-0036, contracts/contract-version, contracts/phase-ownership, docs/25 | MI4-02 base round-trip/mismatch and source-export tests; MI4-04 implements SafeVector closure; MI4-11/12 add JPEG/CFF separately; MI4-13 freezes old bytes plus complete M4 branches |
...
| limits | `ResourceLimits` / `MachineInputLimitBounds` plus private `M4ResourceLimits` target | config and capability Schemas | docs/03,07,09,10,18,19,25 | exact field set + package-byte/JSON-depth default/maximum identity + inclusive max semantics + iterative nesting precheck + `I9100`/`I9101`; ADR-0030 maps footnote work, ADR-0031 advanced pagination, ADR-0034 navigation, ADR-0035 generated structure/depth/MCR/artifact/string/MCID/PDF/output, and ADR-0036 reuses JPEG byte/pixel/decode bounds while reserving six private table/glyph/subroutine/operation/outline/subset limits and `R7130`-`R7135` for MI4-12 |
| effective config/build manifest | `EffectiveConfig` / `BuildInputProfile` / `PackageInputRecord` / publication contexts | current 1.3 config/build Schema with conditional `advanced_pagination`; private target 1.4 adds production media/math, MI4-07 metadata/language/navigation facts, MI4-09 tagged observation/evidence facts, and reserves ADR-0036 JPEG/CFF/resource-limit facts for MI4-11/12 | docs/16,19,25, ADR-0031 through ADR-0036 | current raw 1.0/1.1/1.2/1.3 normalization and terminal publication; MI4-13 must preserve old raw-contract/profile 1.3 bytes while atomically adding complete 1.4 version dispatch and forbidding M4 fields in old-profile branches |
...
their transforms/subset/PDF plans, limits, and exact dependencies. MI4-02
through MI4-12 may implement only private contract-1.4
