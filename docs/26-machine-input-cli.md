...
This document is the normative producer guide for the public M1 machine-input
...
mode. The current machine profile is
`typaxis.machine-pdf/paragraph-1`, and it is the default when `--profile` is
omitted.
...
Source and resource roots are intentionally separate. M1 accepts exactly one
...
Example build with every M1 sidecar:
...
Machine diagnostics are canonical JSON under `typaxis.contract/1.1`. A primary
...
| `L5100`, `L5101` | unsupported document or style/master capability |
| `R7100` | unsupported image/resource capability |
...
contract/canonical hash facts after decode. `inputs` contains only successfully
...
after layout selection. A built manifest binds the file/stdout sink, PDF bytes,
...
It clean-builds the current worktree, validates all JSON artifacts, compares
