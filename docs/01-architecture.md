...
reference workspaceのtransition trust境界はpackage自身がcanonical state-0 overlay `G_0`を発行する。list/footnote markerはvalidated ASTからmaterializeし、page/counter referenceとbreak discretionary siteはcanonical empty bytesで開始する。次passのsealed transitionはmaterialized predecessorのexact placed-anchor setからpage-referenceをchecked 1-based physical page numberへ解決し、それ以外のcanonical bytesを保持する。新overlayはtransition receiptが所有するためcaller-supplied `GeneratedTextStore`をinitial seedまたは次passへ差し替えるfixture APIは提供しない。

`DisplayDocument`まではPDF非依存である。`LateResourceFinalizer`以降はPDF-specific phaseであり、`FrozenPdfResourcePlans`はPDF-readyだがbackend identity-freeである。すなわちCIDとprofile型ごとのCID/GID mapping（current CIDFontType2ではCIDToGIDMap、ADR-0036のnon-current CIDFontType0/CFF1ではcanonical charsetによるdirect mapping）、FontDescriptor metrics、PDF image encoding policyを持てる一方、backend handle、PDF resource name、PDF object IDは持たない。
