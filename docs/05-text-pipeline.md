...
paragraph bidi resolutionで得たparagraph levelはcanonical itemizerが`ItemizedShapeRequests`へ、各runのembedding levelはimmutable shaped run sliceへ保持する。line layoutはpackage receiptとshaped runを別々に再結合せず、exact itemizer ownerにbindした`ParagraphShapedText`だけを受理する。break確定後、各lineでUAX #9 L1のline-level reset、final reshape、justification、L2 visual reorderの順に実行する。L1後levelはlogical cluster順のまま保持し、L2は`visual_to_logical` permutationとして分離する。その後のGlyphRunもvisual glyph orderとlogical TextSpanを所有するcluster groupを分離する。RTLでcluster順がvisualに並んでもsource/text mappingを失わない。

paragraph内にparsed/generated text siteが複数ある場合、canonical itemizerはvalidated inline treeから得たexact site列とreceipt列を照合し、UTF-8をlogical順に連結した一つのbidi/script passを行う。site境界がgraphemeを分断する入力は拒否し、各runは元site namespaceのspanへ戻して発行する。run先頭/末尾のpre/post contextは同site prefix/suffix、無ければ直前/直後のnonempty canonical siteを使うため、callerがsiteをreorderまたはcontextへ混入できない。
