...
use super::*;
pub struct ProductionBodyMixedStablePages<'b, 'f, 's, 'p, 'a> {
    sequence: ProductionBodyMixedPageSequence<'b, 'f, 's, 'p, 'a>,
    passes: u16,
    records: u64,
    work: u64,
}
impl<'b, 'f, 's, 'p, 'a> ProductionBodyMixedStablePages<'b, 'f, 's, 'p, 'a> {
    pub fn sequence(&self) -> &ProductionBodyMixedPageSequence<'b, 'f, 's, 'p, 'a> {
        &self.sequence
    }
...
impl<'b, 'f, 's, 'p, 'a> ProductionFootnoteDemandSearch<'b, 'f, 's, 'p, 'a> {
    ...
    ) -> Result<ProductionBodyMixedStablePages<'b, 'f, 's, 'p, 'a>, ProductionBodyPaginationError>
    {
        let projection = page_stability_kernel::converge(self, remaining_passes)?;
        Ok(ProductionBodyMixedStablePages {
    ...
    fn same_mixed_geometry(
        &mut self,
        left: &ProductionBodyMixedPlacedSequence<'_, 'b, 'f, 's, 'p, 'a>,
        right: &ProductionBodyMixedPlacedSequence<'_, 'b, 'f, 's, 'p, 'a>,
        ...
        for (l, r) in left.pages().iter().zip(right.pages()) {
            ...
            if lp.page_index() != rp.page_index()
                ...
                || lp.next_state().next_item() != rp.next_state().next_item()
            ...
            let cursor = |c: Option<ProductionTableCursor<'b, 'f, 's, 'p, 'a>>| {
                c.map(|c| (c.table_index(), c.offset(), c.next_row(), c.is_initial()))
            };
            if cursor(lp.next_state().table_continuation())
                != cursor(rp.next_state().table_continuation())
            {
                return Ok(false);
            }
                ...
                (Some(a), Some(b)) => {
                    ...
                    for (a, b) in a.fragments().iter().zip(b.fragments()) {
                        ...
                        if a.definition_index() != b.definition_index()
                            || a.consumed_range() != b.consumed_range()
            ...
            if !self.same_records(l.fragments(), r.fragments())?
...
{
    type Sequence = ProductionBodyMixedPageSequence<'b, 'f, 's, 'p, 'a>;
