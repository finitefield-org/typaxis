//! Automatic page candidates over ordinary ranges and source-bound table cuts.
use super::*;
use crate::production_body::body_flow::book_v2::BookV2TableCursor;

pub struct BookV2BodyMixedPageState<'b, 'f, 's, 'p, 'a> {
    source: BookV2BodySourceState<'b, 'f, 's, 'p, 'a>,
    page: u32,
    empty_page: bool,
    name: Option<usize>,
}
impl<'b, 'f, 's, 'p, 'a> BookV2BodyMixedPageState<'b, 'f, 's, 'p, 'a> {
    pub fn source_state(&self) -> &BookV2BodySourceState<'b, 'f, 's, 'p, 'a> {
        &self.source
    }
    pub const fn page_index(&self) -> u32 {
        self.page
    }
    pub fn is_complete(&self) -> bool {
        self.source.is_complete() && !self.empty_page
    }
}
pub struct BookV2BodyMixedPageSelection<'b, 'f, 's, 'p, 'a> {
    page: u32,
    selected: BookV2BodyMixedCandidate<'b, 'f, 's, 'p, 'a>,
    forced: Option<NodeId>,
    next: BookV2BodyMixedPageState<'b, 'f, 's, 'p, 'a>,
    attempts: u32,
    body_bounds: Rect,
    declared_footnote_region: Option<Rect>,
    named_page: Option<&'p str>,
    name_index: Option<usize>,
}
impl<'b, 'f, 's, 'p, 'a> BookV2BodyMixedPageSelection<'b, 'f, 's, 'p, 'a> {
    pub fn named_page(&self) -> Option<&'p str> {
        self.named_page
    }
    pub fn named_page_index(&self) -> Option<usize> {
        self.name_index
    }
    pub fn body_bounds(&self) -> Rect {
        self.body_bounds
    }
    pub fn declared_footnote_region(&self) -> Option<Rect> {
        self.declared_footnote_region
    }
    pub const fn page_index(&self) -> u32 {
        self.page
    }
    pub fn candidate(&self) -> &BookV2BodyMixedCandidate<'b, 'f, 's, 'p, 'a> {
        &self.selected
    }
    pub fn next_state(&self) -> &BookV2BodyMixedPageState<'b, 'f, 's, 'p, 'a> {
        &self.next
    }
    pub const fn forced_break(&self) -> Option<NodeId> {
        self.forced
    }
    pub const fn candidate_attempts(&self) -> u32 {
        self.attempts
    }
}
pub struct BookV2BodyMixedPageSequence<'b, 'f, 's, 'p, 'a> {
    owner_id: u64,
    measurements_fingerprint: [u8; 32],
    pages: Vec<BookV2BodyMixedPageSelection<'b, 'f, 's, 'p, 'a>>,
}
impl<'b, 'f, 's, 'p, 'a> BookV2BodyMixedPageSequence<'b, 'f, 's, 'p, 'a> {
    /// Identity of the source-bound cell measurements used by this search.
    /// Page selection is still authorized by the issuing search owner.
    pub const fn measurements_fingerprint(&self) -> [u8; 32] {
        self.measurements_fingerprint
    }
    pub fn pages(&self) -> &[BookV2BodyMixedPageSelection<'b, 'f, 's, 'p, 'a>] {
        &self.pages
    }
}
type MixedBoundary<'b, 'f, 's, 'p, 'a> = page_ranking::Boundary<
    BookV2BodyCandidatePart<'b, 'f, 's, 'p, 'a>,
    (i64, usize, usize, i64, usize),
>;
struct Alternatives<'b, 'f, 's, 'p, 'a> {
    candidates: Vec<MixedBoundary<'b, 'f, 's, 'p, 'a>>,
}
impl<'b, 'f, 's, 'p, 'a> BookV2FootnoteDemandSearch<'b, 'f, 's, 'p, 'a> {
    // Outer None is body completion; inner None is the actual unnamed source.
    fn source_page_name(&self, item: usize, table: usize) -> Option<Option<usize>> {
        if let Some(index) = self.tables.as_ref().and_then(|t| t.at(item, table)) {
            Some(self.content.flow.table_page_name_index(index))
        } else if item < self.content.flow.body_items().len() {
            Some(self.content.flow.body_page_name_index(item))
        } else {
            None
        }
    }

    pub fn begin_mixed_pages(
        &mut self,
    ) -> Result<BookV2BodyMixedPageState<'b, 'f, 's, 'p, 'a>, ProductionBodyPaginationError> {
        if self.tables.is_none() {
            return Err(error(NodeId::new(0), E::ReceiptMismatch));
        }
        let mut index = 0;
        let mut table_index = 0;
        let items = self.content.flow.body_items();
        while index < items.len() || table_index < self.body_table_count() {
            self.content.step(NodeId::new(0))?;
            self.content.step(NodeId::new(0))?;
            if let Some(table) = self.tables.as_ref().unwrap().at(index, table_index) {
                let context = self.tables.as_ref().unwrap();
                let end = context.range(table).expect("bound table").end;
                if context.table(table).keep_with_next()
                    && context.at(end, context.successor(table)).is_none()
                    && items.get(end).is_some_and(|i| i.source.is_none())
                {
                    return Err(error(
                        context.table(table).owner(),
                        E::KeepAcrossForcedBreak,
                    ));
                }
                if context.table(table).keep_with_next()
                    && self
                        .source_page_name(end, context.successor(table))
                        .is_some_and(|n| Some(n) != self.source_page_name(index, table_index))
                {
                    return Err(error(
                        context.table(table).owner(),
                        E::KeepAcrossForcedBreak,
                    ));
                }
                index = end;
                table_index = context.successor(table_index);
            } else {
                if items[index].keep
                    && self
                        .tables
                        .as_ref()
                        .unwrap()
                        .at(index + 1, table_index)
                        .is_none()
                    && items.get(index + 1).is_some_and(|i| i.source.is_none())
                {
                    return Err(error(items[index].owner, E::KeepAcrossForcedBreak));
                }
                if items[index].keep
                    && self
                        .source_page_name(index + 1, table_index)
                        .is_some_and(|n| Some(n) != self.source_page_name(index, table_index))
                {
                    return Err(error(items[index].owner, E::KeepAcrossForcedBreak));
                }
                index += 1;
            }
        }
        self.content.charge.take(1, NodeId::new(0))?;
        Ok(BookV2BodyMixedPageState {
            source: self.begin_body_source()?,
            page: 0,
            empty_page: true,
            name: None,
        })
    }
    fn mixed_boundary_key(
        &self,
        end: usize,
        next_table: usize,
        continuation: Option<BookV2TableCursor<'b, 'f, 's, 'p, 'a>>,
        last_items: Option<std::ops::Range<usize>>,
        used: Length,
        forced: bool,
    ) -> Result<(i64, usize, usize, i64, usize), ProductionBodyPaginationError> {
        let items = self.content.flow.body_items();
        let terminal = forced
            || self
                .source_page_name(end, next_table)
                .is_some_and(|name| name != self.active_page_name)
            || (continuation.is_none()
                && self.tables.as_ref().unwrap().at(end, next_table).is_none()
                && items.get(end).is_none_or(|i| i.source.is_none()));
        let height = fit_kernel::FitSearch::body(self).height().get();
        let cost = page_ranking::cost(
            items,
            last_items,
            used,
            height,
            &self.content.paragraph_lengths,
            &self.content.headings,
            terminal,
        )?;
        Ok((
            cost,
            end,
            next_table,
            continuation.map_or(0, |c| c.offset().raw()),
            continuation.map_or(0, |c| c.next_caption_item()),
        ))
    }
    fn queue_mixed(
        &mut self,
        requests: &[BookV2BodyCandidatePart<'b, 'f, 's, 'p, 'a>],
        key: (i64, usize, usize, i64, usize),
        alternatives: &mut Alternatives<'b, 'f, 's, 'p, 'a>,
    ) -> Result<(), ProductionBodyPaginationError> {
        page_ranking::queue(
            requests,
            key,
            &mut alternatives.candidates,
            self.content.maximum_candidates,
            &mut self.content.charge,
            &mut self.content.steps,
            self.content.maximum_steps,
        )
    }
    /// Enumerate complete ordinary/table prefixes and each legal final table
    /// cut before testing footnotes in cost order. Exhausting enumeration is
    /// still an error. After the first fit, every remaining key is dominated.
    pub fn select_mixed_page(
        &mut self,
        state: &BookV2BodyMixedPageState<'b, 'f, 's, 'p, 'a>,
    ) -> Result<
        Option<BookV2BodyMixedPageSelection<'b, 'f, 's, 'p, 'a>>,
        ProductionBodyPaginationError,
    > {
        self.verify_state(&state.source.demand)?;
        if state.is_complete() {
            return Ok(None);
        }
        let frames = self.content.flow.lines().frames().expect("bound frames");
        let name = self
            .source_page_name(state.source.item, state.source.next_table)
            .unwrap_or(state.name);
        let selected = frames
            .page_plan()
            .map(|plan| plan.named_page(state.page, name))
            .transpose()
            .map_err(|_| error(NodeId::new(0), E::PageLimit))?;
        self.active_page_frames = selected;
        self.active_page_name = name;
        let result = self.select_mixed_page_in_frames(state);
        self.active_page_frames = None;
        self.active_page_name = None;
        result
    }
    fn select_mixed_page_in_frames(
        &mut self,
        state: &BookV2BodyMixedPageState<'b, 'f, 's, 'p, 'a>,
    ) -> Result<
        Option<BookV2BodyMixedPageSelection<'b, 'f, 's, 'p, 'a>>,
        ProductionBodyPaginationError,
    > {
        self.verify_state(&state.source.demand)?;
        if state.is_complete() {
            return Ok(None);
        }
        let root = NodeId::new(0);
        if state.page >= self.maximum_pages {
            return Err(error(root, E::PageLimit));
        }
        self.content.charge.take(1, root)?;
        let items = self.content.flow.body_items();
        let maximum = fit_kernel::FitSearch::body(self).height().get();
        let mut alternatives = Alternatives {
            candidates: Vec::new(),
        };
        let mut requests = Vec::new();
        let mut index = state.source.item;
        let mut table_index = state.source.next_table;
        let mut used = Length::ZERO;
        let mut after = Length::ZERO;
        let mut ordinary = false;
        let mut ordinary_start = state.source.item;
        loop {
            self.content.step(root)?;
            self.content.step(NodeId::new(0))?;
            if self
                .source_page_name(index, table_index)
                .is_some_and(|name| name != self.active_page_name)
            {
                break;
            }
            if self
                .tables
                .as_ref()
                .unwrap()
                .at(index, table_index)
                .is_some()
            {
                let table_cursor = match state
                    .source
                    .continuation
                    .filter(|_| table_index == state.source.next_table)
                {
                    Some(c) => c,
                    None => self.begin_table(table_index)?,
                };
                let context = self.tables.as_ref().unwrap();
                let table = context.table(table_index);
                let gap = if requests.is_empty() {
                    Length::ZERO
                } else {
                    add(after, table.space_before(), table.owner())?
                };
                let top = add(used, gap, table.owner())?;
                if top > maximum {
                    break;
                }
                let mut capacity = maximum
                    .checked_sub(top)
                    .ok_or_else(|| error(root, E::ArithmeticOverflow))?;
                self.content.charge.take(1, root)?;
                requests
                    .try_reserve(1)
                    .map_err(|_| error(root, E::AllocationFailure))?;
                requests.push(BookV2BodyCandidatePart::Table {
                    cursor: table_cursor,
                    capacity,
                });
                let mut complete = None;
                loop {
                    let context = self.tables.as_mut().unwrap();
                    let selected = context.evaluate(
                        &table_cursor,
                        capacity,
                        self.headers,
                        self.active_page_frames.map(|f| f.body()).map(|r| r.width()),
                        &mut self.content.charge,
                        &mut self.content.steps,
                    )?;
                    let Some(selected) = selected else {
                        break;
                    };
                    let smaller = self.tables.as_mut().unwrap().earlier_capacity(
                        &table_cursor,
                        &selected,
                        &mut self.content.steps,
                    )?;
                    let terminal = selected.after().is_terminal();
                    if terminal {
                        complete = Some((capacity, selected.used_height()));
                    }
                    let context = self.tables.as_ref().unwrap();
                    let end = context.range(table_index).expect("bound table").end;
                    if !terminal
                        || !context.table(table_index).keep_with_next()
                        || (end == items.len()
                            && context.successor(table_index) == self.body_table_count())
                    {
                        *requests.last_mut().unwrap() = BookV2BodyCandidatePart::Table {
                            cursor: table_cursor,
                            capacity,
                        };
                        let next = if terminal { end } else { index };
                        let continuation = (!terminal).then_some(selected.after());
                        let key = self.mixed_boundary_key(
                            next,
                            if terminal {
                                context.successor(table_index)
                            } else {
                                table_index
                            },
                            continuation,
                            None,
                            add(top, selected.used_height(), root)?,
                            selected.forced_break_owner().is_some(),
                        )?;
                        self.queue_mixed(&requests, key, &mut alternatives)?;
                    }
                    let Some(next) = smaller else {
                        break;
                    };
                    if next >= capacity {
                        return Err(error(root, E::ReceiptMismatch));
                    }
                    capacity = next;
                }
                let Some((capacity, height)) = complete else {
                    break;
                };
                *requests.last_mut().unwrap() = BookV2BodyCandidatePart::Table {
                    cursor: table_cursor,
                    capacity,
                };
                used = add(top, height, root)?;
                let context = self.tables.as_ref().unwrap();
                after = context.table(table_index).space_after();
                index = context.range(table_index).expect("bound table").end;
                table_index = context.successor(table_index);
                ordinary = false;
                continue;
            }
            let Some(item) = items.get(index).filter(|i| i.source.is_some()) else {
                break;
            };
            let gap = if requests.is_empty() {
                Length::ZERO
            } else {
                add(after, item.before, item.owner)?
            };
            let next = add(add(used, gap, item.owner)?, item.consumed()?, item.owner)?;
            if next > maximum {
                break;
            }
            if !ordinary {
                ordinary_start = index;
                self.content.charge.take(1, item.owner)?;
                requests
                    .try_reserve(1)
                    .map_err(|_| error(item.owner, E::AllocationFailure))?;
                requests.push(BookV2BodyCandidatePart::Items { end: index + 1 });
                ordinary = true;
            } else {
                *requests.last_mut().unwrap() = BookV2BodyCandidatePart::Items { end: index + 1 };
            }
            used = next;
            after = item.after;
            index += 1;
            if !item.keep || (index == items.len() && table_index == self.body_table_count()) {
                let key = self.mixed_boundary_key(
                    index,
                    table_index,
                    None,
                    Some(ordinary_start..index),
                    used,
                    false,
                )?;
                self.queue_mixed(&requests, key, &mut alternatives)?;
            }
        }
        let mut best = None;
        let mut attempts = 0u32;
        let candidate_count = alternatives.candidates.len() as u32;
        for boundary in alternatives.candidates {
            attempts = attempts
                .checked_add(1)
                .ok_or_else(|| error(root, E::FootnoteSearchLimit))?;
            if attempts > u32::from(self.maximum_reflows) {
                return Err(error(root, E::FootnoteSearchLimit));
            }
            self.content.step(root)?;
            if let Some(candidate) =
                self.evaluate_mixed_candidate(&state.source, &boundary.requests)?
            {
                let key = self.mixed_boundary_key(
                    candidate.next_state().next_item(),
                    candidate.next_state().next_table_index(),
                    candidate.next_state().table_continuation(),
                    candidate.parts().last().and_then(|p| p.items()),
                    candidate.used_height(),
                    candidate
                        .parts()
                        .last()
                        .and_then(|p| p.table())
                        .is_some_and(|t| t.forced_break_owner().is_some()),
                )?;
                if key != boundary.key {
                    return Err(error(root, E::ReceiptMismatch));
                }
                best = Some(candidate);
                break;
            }
        }
        if best.is_none()
            && (!state.source.demand.pending.is_empty()
                || (self
                    .tables
                    .as_ref()
                    .unwrap()
                    .at(state.source.item, state.source.next_table)
                    .is_none()
                    && items
                        .get(state.source.item)
                        .is_none_or(|i| i.source.is_none())))
        {
            let observed = candidate_count + 1;
            if observed > u32::from(self.content.maximum_candidates) {
                return Err(error(
                    root,
                    E::PageBreakLookbackLimit {
                        limit: self.content.maximum_candidates,
                        observed,
                    },
                ));
            }
            requests.clear();
            attempts = attempts
                .checked_add(1)
                .ok_or_else(|| error(root, E::FootnoteSearchLimit))?;
            if attempts > u32::from(self.maximum_reflows) {
                return Err(error(root, E::FootnoteSearchLimit));
            }
            best = self.evaluate_mixed_candidate(&state.source, &requests)?;
        }
        let Some(selected) = best else {
            return Ok(None);
        };
        let end = selected.next_state().next_item();
        let next_table = selected.next_state().next_table_index();
        let table = selected.next_state().table_continuation();
        let outside_forced = if table.is_none()
            && self.tables.as_ref().unwrap().at(end, next_table).is_none()
            && self.source_page_name(end, next_table) == Some(self.active_page_name)
        {
            items
                .get(end)
                .filter(|i| i.source.is_none())
                .map(|i| i.owner)
        } else {
            None
        };
        let forced = selected
            .parts()
            .last()
            .and_then(|p| p.table())
            .and_then(|t| t.forced_break_owner())
            .or(outside_forced);
        let footnote_forced = selected
            .footnotes()
            .is_some_and(|f| f.forced_break_owner().is_some());
        self.content.charge.take(1, root)?;
        let next = BookV2BodyMixedPageState {
            source: BookV2BodySourceState {
                demand: self.fork(selected.next_state().demand(), 0)?,
                item: end + usize::from(outside_forced.is_some()),
                next_table,
                table_end: state.source.table_end,
                continuation: table,
            },
            page: state
                .page
                .checked_add(1)
                .ok_or_else(|| error(root, E::PageLimit))?,
            empty_page: forced.is_some() || footnote_forced,
            name: self.active_page_name,
        };
        Ok(Some(BookV2BodyMixedPageSelection {
            page: state.page,
            selected,
            forced,
            next,
            attempts,
            body_bounds: fit_kernel::FitSearch::body(self),
            declared_footnote_region: fit_kernel::FitSearch::footnote_region(self),
            named_page: self.active_page_name.and_then(|index| {
                self.content
                    .flow
                    .lines()
                    .frames()
                    .and_then(|f| f.page_plan())
                    .and_then(|p| p.name(index))
            }),
            name_index: self.active_page_name,
        }))
    }
    pub fn select_mixed_pages(
        &mut self,
    ) -> Result<BookV2BodyMixedPageSequence<'b, 'f, 's, 'p, 'a>, ProductionBodyPaginationError>
    {
        let root = NodeId::new(0);
        self.content.charge.take(1, root)?;
        let initial = self.begin_mixed_pages()?;
        let mut pages: Vec<BookV2BodyMixedPageSelection<'b, 'f, 's, 'p, 'a>> = Vec::new();
        loop {
            let state = pages.last().map_or(&initial, |p| p.next_state());
            if state.is_complete() {
                break;
            }
            let selected = self
                .select_mixed_page(state)?
                .ok_or_else(|| error(root, E::JointPageNoFit))?;
            let next = selected.next_state();
            let table_progress = next.source.next_table > state.source.next_table
                || match (state.source.continuation, next.source.continuation) {
                    (None, Some(c)) => !c.is_initial(),
                    (Some(a), Some(b)) => {
                        b.offset() > a.offset()
                            || b.next_row() > a.next_row()
                            || b.next_caption_item() > a.next_caption_item()
                            || b.cell_progress_fingerprint() != a.cell_progress_fingerprint()
                            || (!a.has_started_rows() && b.has_started_rows())
                    }
                    (Some(_), None) => true,
                    (None, None) => false,
                };
            let mut notes = false;
            if let Some(region) = selected.candidate().footnotes() {
                for fragment in region.fragments() {
                    self.content.step(root)?;
                    let fragment = fragment.fragment();
                    if let Some(mixed) = fragment.mixed() {
                        for part in mixed.parts() {
                            self.content.step(root)?;
                            notes |= part.items().is_some_and(|range| !range.is_empty());
                            if let Some(table) = part.table() {
                                let a = table.before();
                                let b = table.after();
                                notes |= a.is_terminal() != b.is_terminal()
                                    || a.offset() != b.offset()
                                    || a.next_row() != b.next_row()
                                    || a.next_caption_item() != b.next_caption_item()
                                    || a.cell_progress_fingerprint()
                                        != b.cell_progress_fingerprint();
                            }
                        }
                    } else {
                        notes |= !fragment.consumed_range()?.is_empty();
                    }
                }
            }
            if next.source.item < state.source.item
                || next.source.next_table < state.source.next_table
                || (next.source.item == state.source.item
                    && !table_progress
                    && !notes
                    && !(state.empty_page && next.is_complete()))
            {
                return Err(error(root, E::ReceiptMismatch));
            }
            pages
                .try_reserve(1)
                .map_err(|_| error(root, E::AllocationFailure))?;
            pages.push(selected);
        }
        if pages.is_empty() {
            return Err(error(root, E::ReceiptMismatch));
        }
        Ok(BookV2BodyMixedPageSequence {
            owner_id: self.owner_id,
            measurements_fingerprint: self.tables.as_ref().unwrap().measurements_fingerprint(),
            pages,
        })
    }
    pub fn verify_mixed_sequence(
        &self,
        sequence: &BookV2BodyMixedPageSequence<'_, '_, '_, '_, '_>,
    ) -> Result<(), ProductionBodyPaginationError> {
        if sequence.owner_id != self.owner_id
            || self.tables.as_ref().is_none_or(|tables| {
                sequence.measurements_fingerprint != tables.measurements_fingerprint()
            })
            || sequence
                .pages
                .last()
                .is_none_or(|p| !p.next_state().is_complete())
        {
            return Err(error(NodeId::new(0), E::ReceiptMismatch));
        }
        Ok(())
    }
}
