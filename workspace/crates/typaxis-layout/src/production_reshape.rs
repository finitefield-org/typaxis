...
fn selected_state(
    ...
    selected_fingerprint_state(lines.fingerprint())
}

pub(super) fn selected_fingerprint_state(
    fingerprint: [u8; 32],
) -> Result<LineLayoutStateFingerprint, BreakError> {
...
pub fn with_converged_production_body_lines<R>(
    ...
    with_converged_production_body_lines_with_native_context(
        package,
        navigation,
        profile,
        limits,
...
/// Reuse the outer build owner across both line feedback and page-label passes.
/// Every selected native item borrows its original computation receipt.
#[allow(clippy::too_many_arguments)]
pub fn with_converged_production_body_lines_with_native_context<R>(
    package: &ValidatedStagingSemanticPackage,
    navigation: &ValidatedStagingBookNavigationV2,
    profile: &StagingPrecomposedVectorProfileAuthorization,
    limits: &M4EffectiveResourceLimits,
    ...
    body: Rect,
    max_candidate_steps: u64,
    native_math: Option<&ProductionNativeMathContext<'_>>,
    use_stable: impl FnOnce(ProductionConvergedBodyLines<'_, '_, '_>) -> R,
) -> Result<R, ProductionBodyReshapeError> {
    ...
    let (mut contexts, initial_state) = {
        ...
        let prepared = prepare_production_inline_items_with_native_context(
            package,
            navigation,
            profile,
            limits,
