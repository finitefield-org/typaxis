...
pub(super) fn observe_common_tagged_graph(
    ...
    vector_records
        .try_reserve_exact(usize::try_from(vector_count).map_err(|_| E::RecordLimit)?)
        .map_err(|_| E::AllocationFailure)?;
    for group in structure.groups() {
        let Some(usage_id) = group.vector_usage_id() else {
            continue;
        };
        if usage_id as usize != vector_records.len() {
    ...
    if vector_records.len() as u64 != vector_count {
        return Err(E::ReceiptMismatch);
    }
    let mut canonical_jcs = String::from("{\"algorithm\":\"typaxis.production-common-tagged-pdf-observation/1\",\"marked_content_sha256\":");
    push_observed_hash(&mut canonical_jcs, marked_content_sha256);
